...
- (void)layoutContent:(NSArray *)array
{
    ...
    for (int i = 0; i < array.count; i ++) {
        TagEntity *entity = array[i];
        NSString *title = entity.title;
        CGSize size = [title boundingRectWithSize:CGSizeMake(300, CGFLOAT_MAX) options:(NSStringDrawingTruncatesLastVisibleLine | NSStringDrawingUsesFontLeading  |NSStringDrawingUsesLineFragmentOrigin) attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:11.0]} context:nil].size;
        ...
        button.frame = CGRectMake(width, 0, size.width + 30 + title.length * 5, 20);
        
        UIImage *image = [UIImage imageNamed:@"tag@2x.png"];
